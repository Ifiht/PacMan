...
#============No Sleep=============#
# These functions make it impossible to sleep during a night, but return the value to normal after the night is over
execute unless score #day h-nigh.day = #interval h-nigh.config store result score #sleep h-nigh.config run gamerule playersSleepingPercentage
execute store result storage rrc:h-nigh gmrl.slp int 1 run scoreboard players get #sleep h-nigh.config
execute if score #day h-nigh.day matches 0 run function h-nigh:ref/sleep_rule with storage rrc:h-nigh gmrl
...
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] store result score @s h-nigh.vfx_random run random value 1..100
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 20 at @s run particle minecraft:angry_villager ~ ~ ~1
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 40 at @s run particle minecraft:angry_villager ~1 ~ ~
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 60 at @s run particle minecraft:angry_villager ~ ~ ~-1
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 80 at @s run particle minecraft:angry_villager ~-1 ~ ~
execute if score #day h-nigh.day = #interval h-nigh.config as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 100 at @s run particle minecraft:angry_villager ~ ~2 ~
